...
export const inputChecker = ({value, type}:  IInputChecker): string | null => {
    if (type === 'name') {
		if (value.length === 0) return 'Please enter the name'
		if (value.length > 60) return 'Name is too long, please check'
...
export const lockFocusInside = ({_parentElement, initialFocused = 0, returnFocus = true}: ILockFocusInside): ILockFocusInsideOutput => {
	...
	return {
        ...
        rebuild: () => {  //in case all the content inside _parentElement was changed
